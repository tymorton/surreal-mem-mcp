...
pub async fn call_tool(params: Value, db: Arc<SurrealClient>) -> Value {
    ...
    match name {
        ...
        "check_index_status" => {
            ...
            let file_count = query_res.as_mut().and_then(|r| r.take::<Vec<Value>>(0).ok())
                .unwrap_or_default()
                .first()
                .and_then(|row| row.get("file_count"))
                .and_then(|v| v.as_u64())
                .unwrap_or(0);

            let last_indexed_at = query_res.as_mut().and_then(|r| r.take::<Vec<Value>>(1).ok())
                .unwrap_or_default()
                .first()
                .and_then(|row| row.get("indexed_at"))
                .cloned()
                .unwrap_or(json!(null));
